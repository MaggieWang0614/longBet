longBet <- function(y, x, z, t0, mc = 100, burnin = 10, ntrees = 10,
                    ...
                    a = 16, b = 4){
  # y: n*t matrix of longitudinal data
  # x: n*p matrix of static covariates
  # z: n*1 vector of treatment variables
  # T0: treatment time
  ...
  for (iter in 1:mc){
    ...
    res[,t0:t1] = res[, t0:t1] + matrix(rep(tau_vec * z, t1-t0+1), n, t1-t0+1)
    # a demo model only use mean(res) to use XBCF directly.
    xbcf_y = rowMeans(res[,t0:t1])
    # xbcf_y = res[,t0] # if this works well, we can directly estimate time-varying treatment effect.
    xbcf.fit = XBCF(as.matrix(xbcf_y), as.matrix(z), as.matrix(x), as.matrix(x),
